...
No top-level DMA section — maps via `Cheat` (~2581) DMA lanes and `Anti Cheat > Detection:DMA` (~610), plus hypervisor/virtualization/HWID detection and `Windows Security Features` (~9; TPM/IOMMU/VBS/HVCI attestation samples). (source: wiki/sources/README-categories.md)
